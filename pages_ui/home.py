...
def render_home():
    ...
    with col3:
        st.metric("🏟️ Matches Recorded", m_count)
    with col4:
    ...
    st.subheader("🗄️ How to Connect Your SQL Database")
    st.markdown("""
    Cricbuzz LiveStats is **database-agnostic** and operates seamlessly on **SQLite**, **PostgreSQL**, and **MySQL**.
    Use the **⚙️ DB Settings** tab in the sidebar or edit your `.env` configuration file.
    """)
    ...
    with tabs[1]:
        ...
        ```
        2. Set the environment variables in `.env` (or via the **⚙️ DB Settings** page):
        ```env
        DB_TYPE=postgresql
        DB_HOST=localhost
        DB_PORT=5432
    ...
    with tabs[2]:
        ...
        CREATE DATABASE cricket_analytics;
        ```
        2. Set the environment variables in `.env` (or via **⚙️ DB Settings**):
        ```env
        DB_TYPE=mysql
        DB_HOST=localhost
        DB_PORT=3306
